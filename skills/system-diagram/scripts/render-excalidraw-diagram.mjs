...
const sourceText = readFileSync(inputPath, 'utf8').trim();
const sourceDigest = createHash('sha256').update(sourceText).digest('hex');
const spec = normalizeSpec(JSON.parse(sourceText));
const renderedSvg = await renderExcalidrawDiagram(spec);
const svg = renderedSvg.replace('<!-- svg-source:excalidraw -->', `<!-- svg-source:excalidraw --><!-- svg-spec-sha256:${sourceDigest} -->`);
...
async function renderExcalidrawDiagram(normalizedSpec) {
  const bundle = await buildBrowserExporter();
  const executablePath = chromium.executablePath();
  if (!existsSync(executablePath)) {
    throw new Error(`Pinned Playwright Chromium is missing. Run \`npm install --prefix ${root}\` to install this skill and its browser.`);
  }
  ...
  try {
    ...
    const svg = await page.evaluate((spec) => window.renderExcalidrawDiagram(spec), normalizedSpec);
...
async function startAssetServer() {
  const assetRoot = dirname(require.resolve('@excalidraw/excalidraw'));
