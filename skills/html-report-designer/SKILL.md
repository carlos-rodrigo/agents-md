...
- **Continuity-first UI options** — inspect the existing product before drawing. If it has a UI, reproduce its recognizable shell, navigation, hierarchy, density, control language, and state treatment in every option; compare placement or workflow, not unrelated redesigns. If it has no UI, define the missing shell, responsive hierarchy, controls, and key states in more detail.
- **Decision-first mockup scope** — name the next visual choice and reuse sufficient existing wireframes. Do not add high-resolution or Excalidraw variants unless they reduce a real review uncertainty. Non-visual policy choices need selectors, not screens.
- **Smallest Excalidraw UI comparison** — when an additional visual comparison is needed, use one shared-shell scene with 2-3 highlighted deltas and one common action/outcome path. Keep detailed states in the PRD wireframes. Render and inline the SVG with searchable text, accessibility metadata, stable review IDs, and reading-order reveals.
- **Wireframe-first UI options** — product-facing UI choices should use screen-like wireframes, not plain text boxes. Include the real existing shell when present, representative copy, controls, state chips, empty/loading/error/success states when relevant, and enough structure for the reviewer to comment on specific regions. Each option must also include a short numbered use flow where every step states the user action and expected visible outcome.
...
- **Authentic Excalidraw diagrams** — use the build-time Excalidraw renderer (`scripts/render-excalidraw-diagram.mjs`) for architecture, slice, domain, workflow, and justified UI decision-comparison diagrams. Its explicitly positioned scene exports self-contained inline SVG with coordinated hand-drawn geometry, embedded Virgil, searchable text, stable review IDs, accessibility metadata, and reading-order group reveals.
...
- for user-facing PRDs, anchored existing-UI evidence and mockup-need decision plus a post-story flow for every story and, only when needed, 2-3 continuity-preserving UI options with steps and outcomes;
...
prd-experience-contract     # selected PRD UI option, post-story flows, visible states, review gaps
...
interface-implementation    # components/surfaces, state model, libraries/styles/motion, accessibility
architecture-overview       # high-level diagram with foreground edge labels and reading-order motion
architecture-delta          # compact list/cards of added/changed components
slice-plan                  # PRD-derived vertical slices as cards
domain-interactions         # entity ownership, interaction verbs, state/effect transitions, invariants, and open domain gaps
