...
export function reviewId(spec, type, id, explicit) {
  const namespace = [spec.reviewPrefix, spec.id].filter(Boolean).join('.');
  const suffix = explicit ?? `${type}${id ? `.${id}` : ''}`;
...
function validateFont(font, label) {
  if (!EMBEDDABLE_FONTS.has(font)) {
    throw new Error(`${label} fontFamily must be virgil or cascadia; local fonts are not self-contained`);
  }
}
